...
#+END_SRC
#+CAPTION: Observed cumulative distributions over completion time (unsatisfiable instances) for initialisation strategy 0 (random) and 1 (greedy).
#+LABEL: fig:unsat-greedy
#+RESULTS:
